...
(function () {
  ...
  const ALGORITHM_VERSION = 'v0.98';  // ★ v0.98 — P1 첫화면 구속 KPI 카드 모바일 overflow fix: _kboMetricCard에서 unit이 number와 같은 fontSize(64px)로 렌더링되던 버그 → unit을 number 크기의 40%(예: lg=64→26)로 inline 고정 / em 기반 의존성 제거 / 모바일 unit clamp(11~18px) + flex-wrap + min-width 0 방어 / "147.7 km/h" 같은 결합이 좁은 화면에서 자연 줄바꿈 + grid-cell 폭 초과 방지  // ★ v0.97 — 모바일 가로 스크롤 도입: 좁은 화면에서 강제 stack 대신 좌우 panning으로 정보 손상 없이 보기 / Mannequin SVG (.mannequin-pan-wrap, min-width 720px) + Bell SVG (.bell-pan-wrap, min-width 560px) + P4 5단계 흐름 dot row (.p4-dot-row, flex-wrap: nowrap + scroll-snap) / "← 좌우로 밀어서 전체 보기 →" 힌트 UI 모바일에서만 표시  // ★ v0.96 — 마네킹 라벨 단순화·인과 일관성: 마네킹에서 ELI 영역과 중복되는 4개 박스 삭제 (뒷다리 추진력/착지발 제동력/어깨 파워/팔꿈치 파워) → ELI 6영역 박스(어디서·왜) + 팔꿈치 빨간점 callout(별도 UCL 신호)만 유지 / 하단 legend 정리 / P3 wording 정렬: "주요 손실 구간 — 앞다리 블로킹" → "가장 약한 분절 — 앞다리 블로킹" (P4의 "주요 손실 구간(Result)"과 충돌 해소, 분절 점수 기반 중립 표현) / _P4_LIB landing.leak "가장 큰 누수" → "가장 큰 원인" 정렬  // ★ v0.95 — 마네킹 에너지 흐름 색상 ↔ 콘텐츠 정합: 0~30% 구간(앞다리 path)이 cyan/blue/yellow였던 문제 해결 — leadBlockIssue/leadBlockSevere 종합 변수 도입(lead_leg_braking_impulse · knee_flexion_change_MER_to_BR · lead_knee_ext_change_fc_to_br · Lead_leg_peak_vertical_GRF · br_lead_leg_knee_flexion 평균) → 어스톤 톤 분기 / Front leg fill·alarm circle도 어스톤(#A87333·#A8443A) 통일 / leakBurst 위치 동적 — leadBlockSevere일 때 lKnee 위에 표시 / leak-radial gradient 어스톤 rust로 변경 / 이제 LYH(앞다리 블로킹 19점) 케이스에서 마네킹 시각화가 P3 콘텐츠("주요 손실 구간 — 앞다리 블로킹 19점")와 일치  // ★ v0.94 — P2 단순화 (의사결정자 데모 정보 과부하 해소): 메인 영역은 사분면·Generation/Transmission 점수 카드·Development Opportunity·5축 보드만 노출 / 체력·메카닉·제구 세부 카드 (산식·임계·MLB 평균·코칭·drill)와 분절별 출력 스택은 펼침 details로 격리 + 안내 한 문장 추가  // ★ v0.93 — P3 단순화 (보는 페이지로 정렬): Primary Leak 한 문장 + 3 핵심 수치 카드 (Lead-leg Block · Pelvis→Trunk Transfer · Arm Load Monitor) + Energy Flow Mannequin / Energy Transfer Bar·Coach Delivery·ELI 6영역·ETE 영역별 모두 펼침 details로 격리 / P5 cross-ref 안내 추가
  ...
  function extractScalars(parsed, mass_kg, height_cm) {
    ...
    out.fc_trunk_forward_tilt = _tilt;
    if (ev.FC != null) {
    ...
    if (lkKey && ev.MER != null && ev.BR != null) {
      ...
      if (knee_MER != null && knee_BR != null) {
        // ★ v0.56 — 부호 컨벤션 반전: 양수 = 신전·유지(좋음), 음수 = 굴곡 증가(무너짐)
        //   사용자 직관·메모리 일치 ("무너지지 않으면 좋은 점수")
        out.knee_flexion_change_MER_to_BR = Math.abs(knee_MER) - Math.abs(knee_BR);
      }
    }
